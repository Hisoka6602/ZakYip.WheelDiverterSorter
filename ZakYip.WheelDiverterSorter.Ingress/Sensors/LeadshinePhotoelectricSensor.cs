...
/// </remarks>
public class LeadshinePhotoelectricSensor : ISensor
{
    private readonly ILogger<LeadshinePhotoelectricSensor> _logger;
    ...
    /// 传感器类型
    /// </summary>
    public SensorType Type => SensorType.Photoelectric;
    ...
    public Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("启动雷赛光电传感器 {SensorId}，输入位 {InputBit}", SensorId, _inputBit);
    ...
    public async Task StopAsync()
    {
        ...
        _logger.LogInformation("停止雷赛光电传感器 {SensorId}", SensorId);
    ...
    {
        _logger.LogInformation("雷赛光电传感器 {SensorId} 开始监听", SensorId);
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "雷赛光电传感器 {SensorId} 读取失败", SensorId);
                // 发生错误时等待一段时间再重试
                await Task.Delay(1000, cancellationToken);
            }
        }

        _logger.LogInformation("雷赛光电传感器 {SensorId} 停止监听", SensorId);
    }
