...
public class TcpRuleEngineClientTests : IDisposable
{
    ...
    [Fact]
    public async Task RequestChuteAssignmentAsync_WithValidParcelId_ReturnsResponse()
    {
        // Arrange
...
#pragma warning restore CS0618
    ...
    [Fact]
    public async Task RequestChuteAssignmentAsync_WithTimeout_ThrowsOperationCanceledException()
    {
        // Arrange
        StartTestServerWithDelay(TimeSpan.FromSeconds(10));
        var options = new RuleEngineConnectionOptions
        {
            TcpServer = $"localhost:{_testPort}",
            TimeoutMs = 500,
        ...
        // Act & Assert
        // Timeout is handled via CancellationToken and throws OperationCanceledException
#pragma warning disable CS0618
        await Assert.ThrowsAsync<OperationCanceledException>(() => 
            client.RequestChuteAssignmentAsync(parcelId));
#pragma warning restore CS0618
    }

    [Fact]
    public async Task RequestChuteAssignmentAsync_WithRetry_RetriesOnFailure()
    {
        // Arrange
        var attemptCount = 0;
        StartTestServerWithConditionalResponse(() => ++attemptCount >= 2);
