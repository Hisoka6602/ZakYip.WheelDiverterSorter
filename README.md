...
系统根据路径段 `D1: Straight`，发送数递鸟自定义 TCP 命令：
...
**数递鸟 TCP 请求帧**（直通命令，7 字节）：
```
Byte 1-2: 51 52  // 起始字节（固定）
Byte 3:   57     // 长度字节（固定）
Byte 4:   51     // 设备地址（摆轮 D1）
Byte 5:   52     // 消息类型（控制命令）
Byte 6:   54     // 命令码（回中/直通）
Byte 7:   FE     // 结束字节（固定）
```

**摆轮 D1 响应帧**（应答与完成，7 字节）：
```
Byte 1-2: 51 52  // 起始字节
Byte 3:   57     // 长度字节
Byte 4:   51     // 设备地址（摆轮 D1）
Byte 5:   53     // 消息类型（应答与完成）
Byte 6:   51     // 应答码（命令已接收）
Byte 7:   FE     // 结束字节
```
...
系统根据路径段 `D2: Right`，发送数递鸟自定义 TCP 命令：

**数递鸟 TCP 请求帧**（右摆命令，7 字节）：
```
Byte 1-2: 51 52  // 起始字节（固定）
Byte 3:   57     // 长度字节（固定）
Byte 4:   52     // 设备地址（摆轮 D2）
Byte 5:   52     // 消息类型（控制命令）
Byte 6:   55     // 命令码（右摆/右转 -45°）
Byte 7:   FE     // 结束字节（固定）
```

**摆轮 D2 响应帧**（应答与完成，7 字节）：
```
Byte 1-2: 51 52  // 起始字节
Byte 3:   57     // 长度字节
Byte 4:   52     // 设备地址（摆轮 D2）
