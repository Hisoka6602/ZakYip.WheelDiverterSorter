...
| 入口管理 | 60% | 🚧 | 模拟传感器完成，真实硬件待集成 |
| 调试接口 | 100% | ✅ | 测试环境可用 |
| 可观测性 | 30% | ⚠️ | 仅基础日志 |
| 通信层 | 0% | ❌ | 待开发 |
| 测试覆盖 | 0% | ❌ | 无测试 |

**整体完成度：约 75%**（相比之前的70%有所提升）
...
### 阶段1：真实传感器集成（高优先级） ⏰ 预计2-3周

**目标**：实现真实物理传感器的硬件集成

**任务清单**：
- [ ] **实现真实传感器驱动**（Ingress层）
  - 集成真实光电传感器驱动（参考ZakYip.Singulation）
  - 集成真实激光传感器驱动
...
**修改涉及的文件**：
- `ZakYip.WheelDiverterSorter.Ingress/Sensors/` 新增真实传感器实现
- `ZakYip.WheelDiverterSorter.Ingress/Configuration/` 完善传感器配置
- `appsettings.json` 添加传感器硬件配置
...
|-------|---------|-------------|---------|------|
| 🔴 **最高** | 真实传感器集成 | Ingress层 | 2-3周 | 传感器硬件设备 |
| 🔴 **最高** | 多厂商硬件驱动 | Drivers层扩展 | 2-3周 | PLC设备和驱动 |
| 🔴 **最高** | 通信层开发 | 新增Communication项目 | 3-4周 | RuleEngine已部署 |
| 🟡 **中等** | 整合与测试 | 所有层 | 3-4周 | 前面阶段完成 |
| 🟢 **低** | 生产功能 | 各层 | 持续开发 | 核心功能稳定 |

**总体时间线**：完成核心功能（阶段1-4）预计需要 **10-14周**

**成功标准**：
- ✅ 真实传感器能自动感应包裹并创建记录
- ✅ 支持至少3种主流PLC厂商（雷赛、西门子、三菱/欧姆龙）
...
- **建议**：
  - 集成真实光电传感器驱动（参考ZakYip.Singulation项目）
  - 集成真实激光传感器驱动
  - 实现传感器工厂模式，支持多种传感器品牌
  - 添加传感器状态监控和故障检测
  - 实现传感器校准和调试工具
...
| 风险项 | 描述 | 影响 | 建议 |
|-------|------|------|------|
| 硬件兼容性 | 不同型号PLC协议可能不同 | 驱动失效或功能异常 | 详细测试各型号，建立兼容性列表 |
| 驱动版本冲突 | 不同厂商DLL可能冲突 | 系统崩溃或行为异常 | 使用独立AppDomain或进程隔离 |
| 设备掉线 | 网络中断或设备断电 | 分拣中断，包裹堆积 | 实现自动重连和降级策略 |
| 指令延迟 | PLC响应慢或网络延迟高 | 分拣效率低或超时 | 优化通信协议，添加预读机制 |
| 固件版本差异 | 旧版本PLC可能不支持某些指令 | 功能受限或不可用 | 检查固件版本，提供兼容模式 |
...
| 序号 | 缺失模块 | 描述 | 影响 | 优先级 |
|-----|---------|------|------|--------|
| 1 | **真实传感器驱动** | 仅有模拟传感器，缺少真实硬件传感器驱动集成 | 无法自动感应包裹，必须手动触发 | 🔴 极高 |
| 2 | **通信层** | 完全缺失与RuleEngine.Core的通信客户端 | 无法自动获取格口号，功能不完整 | 🔴 极高 |
| 3 | **多厂商PLC支持** | 仅支持雷赛控制器，不支持西门子/三菱/欧姆龙等 | 限制部署场景，无法在其他PLC环境使用 | 🔴 高 |
| 4 | **并发控制机制** | 缺少摆轮资源锁和包裹队列管理 | 高并发下可能冲突，分拣错误或效率低 | 🔴 高 |
| 5 | **测试框架** | 完全没有单元测试和集成测试 | 代码质量无法保证，回归测试困难 | 🔴 高 |
...
| **Ingress层** | 60% | 模拟传感器、检测服务 | 真实传感器驱动、扫码集成 |
