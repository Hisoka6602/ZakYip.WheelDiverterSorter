# 开发者文档入口

此目录包含项目的内部开发文档。

## 核心文档

| 文档 | 说明 |
|------|------|
| [RepositoryStructure.md](RepositoryStructure.md) | 仓库结构、技术债索引、单一权威实现表 |
| [TechnicalDebtLog.md](TechnicalDebtLog.md) | 技术债详细日志 |
| [DOCUMENTATION_INDEX.md](DOCUMENTATION_INDEX.md) | 完整文档索引 |

## 架构与规范

| 文档 | 说明 |
|------|------|
| [ARCHITECTURE_PRINCIPLES.md](ARCHITECTURE_PRINCIPLES.md) | 架构原则与设计约束 |
| [CODING_GUIDELINES.md](CODING_GUIDELINES.md) | C# 编码规范 |

## 使用指南

| 文档 | 说明 |
|------|------|
| [guides/API_USAGE_GUIDE.md](guides/API_USAGE_GUIDE.md) | API 使用指南 |
| [guides/SYSTEM_CONFIG_GUIDE.md](guides/SYSTEM_CONFIG_GUIDE.md) | 系统配置指南 |
| [guides/UPSTREAM_CONNECTION_GUIDE.md](guides/UPSTREAM_CONNECTION_GUIDE.md) | 上游连接配置 |
| [guides/VENDOR_EXTENSION_GUIDE.md](guides/VENDOR_EXTENSION_GUIDE.md) | 厂商扩展开发 |

## Copilot 阅读顺序

1. `docs/RepositoryStructure.md` - 了解项目结构和技术债
2. `.github/copilot-instructions.md` - 编码规范和约束（相对于仓库根目录）
3. 具体目标文档
