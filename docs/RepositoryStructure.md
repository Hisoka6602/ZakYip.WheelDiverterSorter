...
| **TD-088** | **❌ 未开始** | **🔴 P0-Critical：移除上游路由阻塞等待** → `GetChuteFromUpstreamAsync` 中的同步等待导致延迟累积（3258ms→7724ms），需改为异步非阻塞模式（预计 1-2天，较大重构） | [详情](./TechnicalDebtLog.md#td-088-移除上游路由阻塞等待) |
