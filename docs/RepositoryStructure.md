...
| TD-063 | ❌ 未开始 | 清理旧分拣逻辑和影分身代码 → 删除遗留实现、收敛重复抽象、清理 Legacy 类型（下一个 PR）| [详情](./TechnicalDebtLog.md#td-063-清理旧分拣逻辑和影分身代码) |
| TD-064 | ❌ 未开始 | 系统状态转换到 Running 时初始化所有摆轮为直行 → 添加状态转换钩子、创建摆轮初始化服务（独立 PR）| [详情](./TechnicalDebtLog.md#td-064-系统状态转换到-running-时初始化所有摆轮为直行) |
...
**最近更新**：TD-062（拓扑驱动分拣流程集成）已完成，新增 TD-063（清理旧分拣逻辑和影分身代码）和 TD-064（系统状态转换初始化摆轮）待后续 PR 处理
