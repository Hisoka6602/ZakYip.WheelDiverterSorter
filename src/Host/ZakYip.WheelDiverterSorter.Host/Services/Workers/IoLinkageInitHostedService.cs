...
public sealed class IoLinkageInitHostedService : IHostedService
{
    ...
    private async Task InitializeSiemensS7Async(CancellationToken cancellationToken)
    {
        try
        {
            var s7Options = _s7Connection!.GetType()
                .GetField("_options", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance)?
                .GetValue(_s7Connection) as Drivers.Vendors.Siemens.Configuration.S7Options;

            if (s7Options == null)
            {
                _logger.LogWarning("无法获取 S7 配置信息，跳过初始化");
                return;
            }

            _logger.LogInformation(
                "正在初始化西门子 S7 PLC: {IpAddress}, Rack: {Rack}, Slot: {Slot}, CPU: {CpuType}",
                s7Options.IpAddress, s7Options.Rack, s7Options.Slot, s7Options.CpuType);

            // Ping 检查连通性
            _logger.LogInformation("Ping 检查西门子 S7 PLC: {IP}", s7Options.IpAddress);
            var pingResult = await _connectivityChecker.PingAsync(s7Options.IpAddress, 2000, cancellationToken);

            if (!pingResult.IsReachable)
            {
                _logger.LogWarning(
                    "⚠️ 西门子 S7 PLC 不可达: {IP}，原因: {Error}。跳过连接，IO 操作将返回失败。",
                    s7Options.IpAddress, pingResult.ErrorMessage);
                _logger.LogInformation(
            ...
            _logger.LogInformation(
                ...
                s7Options.IpAddress, pingResult.ResponseTimeMs);
