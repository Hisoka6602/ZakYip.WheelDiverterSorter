...
public class SortingOrchestrator : ISortingOrchestrator, IDisposable
{
    ...
    private async Task ExecuteWheelFrontSortingAsync(long boundWheelDiverterId, long sensorId, int positionIndex, DateTimeOffset triggerTime)
    {
            ...
            && currentTime < earliestDequeueTime)
        {
            ...
            _logger.LogWarning(
                ...
                "包裹 {ParcelId}，DiverterId={DiverterId}",
    ...
    private void UpdateNextPositionExpectedTime(PositionQueueItem currentTask, int currentPositionIndex, DateTime actualArrivalTime)
    {
        ...
        try
        {
            ...
            // 使用 TryUpdateTask 原地更新任务（O(1) 操作，无需出队/入队）
            var updateSuccess = _queueManager.TryUpdateTask(
                nextNode.PositionIndex,
                currentTask.ParcelId,
                dequeuedTask => dequeuedTask with
                {
            ...
            else
            {
                _logger.LogWarning(
                    "[动态时间更新] 包裹 {ParcelId} 在Position {CurrentPos}触发，" +
                    "但无法更新Position {NextPos}的任务（任务可能已被出队）",
                    currentTask.ParcelId,
                    currentPositionIndex,
                    nextNode.PositionIndex);
            }
